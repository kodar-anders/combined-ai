...
- `src/__tests__/` — all test files live here (not colocated with source): `<provider>.test.ts` (mocked unit tests) and `<provider>.integration.test.ts` (live, opt-in). They import the code under test via `../providers/...`. New tests go in this folder and keep the `.test.ts` / `.integration.test.ts` suffix (the latter so the `test:integration` pattern filter matches them).
