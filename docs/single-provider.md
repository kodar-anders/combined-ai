...
> Gemini ample headroom (the default `gemini-3.5-flash` can't switch thinking
> off).
...
`finishReason` lets you tell a truncated or refused answer apart from an empty one
instead of seeing only `text: ""`. A `"length"` reason with empty `text` on Gemini
points to a cap spent on thinking tokens.
...
Pass a per-call `model` or `dimensions` (reduces the output vector size, via
OpenAI `dimensions` / Gemini `outputDimensionality`):
...
are in the registry, billed on input only); Google's embedding endpoint reports
none, so `usage` is omitted there.
